...
DOCKER_TAG?=jaytwo_http_authentication_digest
...
build: restore
	dotnet build ./jaytwo.Http.Authentication.Digest.sln
...
unit-test: build
	...
	rm -rf out/coverage
	cd ./test/jaytwo.Http.Authentication.Digest.Tests; \
		dotnet test \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.Http.Authentication.Digest.Tests.trx"
	reportgenerator \
...
pack:
	rm -rf out/packed
	cd ./src/jaytwo.Http.Authentication.Digest; \
